...
// AuthService Methods:
// 0. NewAuthService(userRepo repository.UserRepository, jwtManager *utils.JWTManager) -> 注入依赖
// 1. Register(ctx context.Context, req *request.RegisterRequest) -> 用户注册
// 2. Login(ctx context.Context, req *request.LoginRequest) -> 用户登录
// 3. Logout(ctx context.Context, userID uint) -> 用户登出
// 4. RefreshToken(ctx context.Context, req *request.RefreshTokenRequest) -> 刷新令牌
// 5. ForgotPassword(ctx context.Context, req *request.ForgotPasswordRequest) -> 忘记密码
// 6. ResetPassword(ctx context.Context, req *request.ResetPasswordRequest) -> 重置密码
// 7. VerifyCode(ctx context.Context, req *request.VerifyCodeRequest) -> 验证码验证
...
type AuthServiceInterface interface {
	Register(ctx context.Context, req *request.RegisterRequest) (*response.RegisterResponse, error)       // 1. 用户注册
	Login(ctx context.Context, req *request.LoginRequest) (*response.AuthResponse, error)                 // 2. 用户登录
	Logout(ctx context.Context, userID uint) error                                                        // 3. 用户登出
	RefreshToken(ctx context.Context, req *request.RefreshTokenRequest) (*response.AuthResponse, error)   // 4. 刷新令牌
	ForgotPassword(ctx context.Context, req *request.ForgotPasswordRequest) (*response.ForgotPasswordResponse, error) // 5. 忘记密码
	ResetPassword(ctx context.Context, req *request.ResetPasswordRequest) (*response.ResetPasswordResponse, error)    // 6. 重置密码
	VerifyCode(ctx context.Context, req *request.VerifyCodeRequest) (*response.VerifyCodeResponse, error) // 7. 验证码验证
}
...
func (s *AuthService) ForgotPassword(ctx context.Context, req *request.ForgotPasswordRequest) (*response.ForgotPasswordResponse, error) {
	// 查找用户
...
func (s *AuthService) ResetPassword(ctx context.Context, req *request.ResetPasswordRequest) (*response.ResetPasswordResponse, error) {
...
// 7. VerifyCode 验证码验证
func (s *AuthService) VerifyCode(ctx context.Context, req *request.VerifyCodeRequest) (*response.VerifyCodeResponse, error) {
	// TODO: 从 Redis 中验证验证码
	// 实际项目中应该:
