...
type FacilityRepository interface {
	List(ctx context.Context, req *request.ListFacilitiesRequest) ([]*model.Facility, int64, error)
	GetByID(ctx context.Context, id uint) (*model.Facility, error)
	Create(ctx context.Context, facility *model.Facility) error
	Update(ctx context.Context, facility *model.Facility) error
...
// List 获取设施列表
func (r *facilityRepository) List(ctx context.Context, req *request.ListFacilitiesRequest) ([]*model.Facility, int64, error) {
	var facilities []*model.Facility
	var total int64

	query := r.db.WithContext(ctx).Model(&model.Facility{})
...
func (r *facilityRepository) Delete(ctx context.Context, id uint) error {
	return r.db.WithContext(ctx).Delete(&model.Facility{}, id).Error
}

// ExistsByID 检查设施是否存在
func (r *facilityRepository) ExistsByID(ctx context.Context, id uint) (bool, error) {
	var count int64
	err := r.db.WithContext(ctx).Model(&model.Facility{}).Where("id = ?", id).Count(&count).Error
	return count > 0, err
}
