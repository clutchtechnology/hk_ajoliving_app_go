...
type NewPropertyRepository interface {
	// Create 创建新楼盘
	Create(ctx context.Context, newProperty *model.NewProperty) error
	// GetByID 根据ID获取新楼盘
	GetByID(ctx context.Context, id uint) (*model.NewProperty, error)
	// Update 更新新楼盘
	Update(ctx context.Context, newProperty *model.NewProperty) error
	// Delete 删除新楼盘
	Delete(ctx context.Context, id uint) error
	// List 获取新楼盘列表
	List(ctx context.Context, req *request.ListNewDevelopmentsRequest) ([]*model.NewProperty, int64, error)
	// GetLayouts 获取新楼盘户型列表
	GetLayouts(ctx context.Context, newPropertyID uint) ([]model.NewPropertyLayout, error)
	// GetImages 获取新楼盘图片列表
	GetImages(ctx context.Context, newPropertyID uint) ([]model.NewPropertyImage, error)
	// GetFeatured 获取精选新楼盘
	GetFeatured(ctx context.Context, limit int) ([]*model.NewProperty, error)
...
// Create 创建新楼盘
func (r *newPropertyRepository) Create(ctx context.Context, newProperty *model.NewProperty) error {
	return r.db.WithContext(ctx).Create(newProperty).Error
}

// GetByID 根据ID获取新楼盘
func (r *newPropertyRepository) GetByID(ctx context.Context, id uint) (*model.NewProperty, error) {
	var newProperty model.NewProperty
...
// Update 更新新楼盘
func (r *newPropertyRepository) Update(ctx context.Context, newProperty *model.NewProperty) error {
	return r.db.WithContext(ctx).Save(newProperty).Error
}

// Delete 删除新楼盘（软删除）
func (r *newPropertyRepository) Delete(ctx context.Context, id uint) error {
	return r.db.WithContext(ctx).Delete(&model.NewProperty{}, id).Error
}

// List 获取新楼盘列表
func (r *newPropertyRepository) List(ctx context.Context, req *request.ListNewDevelopmentsRequest) ([]*model.NewProperty, int64, error) {
	var newProperties []*model.NewProperty
	var total int64

	query := r.db.WithContext(ctx).Model(&model.NewProperty{})
	...
	if req.MinPrice != nil || req.MaxPrice != nil || req.Bedrooms != nil {
		subQuery := r.db.Model(&model.NewPropertyLayout{}).Select("new_property_id")
...
// GetLayouts 获取新楼盘户型列表
func (r *newPropertyRepository) GetLayouts(ctx context.Context, newPropertyID uint) ([]model.NewPropertyLayout, error) {
	var layouts []model.NewPropertyLayout
	err := r.db.WithContext(ctx).
...
// GetImages 获取新楼盘图片列表
func (r *newPropertyRepository) GetImages(ctx context.Context, newPropertyID uint) ([]model.NewPropertyImage, error) {
	var images []model.NewPropertyImage
	err := r.db.WithContext(ctx).
		Where("new_property_id = ?", newPropertyID).
...
// GetFeatured 获取精选新楼盘
func (r *newPropertyRepository) GetFeatured(ctx context.Context, limit int) ([]*model.NewProperty, error) {
	var newProperties []*model.NewProperty
	err := r.db.WithContext(ctx).
		Where("is_featured = ?", true).
		Where("status IN ?", []string{string(model.NewPropertyStatusPresale), string(model.NewPropertyStatusSelling)}).
		Preload("District").
		Preload("Images", func(db *gorm.DB) *gorm.DB {
...
func (r *newPropertyRepository) GetCoverImage(ctx context.Context, newPropertyID uint) (string, error) {
	var image model.NewPropertyImage
	err := r.db.WithContext(ctx).
