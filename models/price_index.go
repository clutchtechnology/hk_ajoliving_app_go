...
type GetPriceIndexRequest struct {
	IndexType    *IndexType `form:"index_type" binding:"omitempty,oneof=overall district estate property_type"`
	DistrictID   *uint      `form:"district_id"`
	EstateID     *uint      `form:"estate_id"`
	PropertyType *string    `form:"property_type"`
	StartPeriod  *string    `form:"start_period" binding:"omitempty,len=7"` // YYYY-MM
	EndPeriod    *string    `form:"end_period" binding:"omitempty,len=7"`   // YYYY-MM
	Page         int        `form:"page,default=1" binding:"min=1"`
	PageSize     int        `form:"page_size,default=20" binding:"min=1,max=100"`
	SortBy       string     `form:"sort_by,default=period"`
	SortOrder    string     `form:"sort_order,default=desc" binding:"omitempty,oneof=asc desc"`
}

// GetPriceTrendsRequest 获取楼价趋势请求
type GetPriceTrendsRequest struct {
	IndexType    *IndexType `form:"index_type" binding:"required,oneof=overall district estate property_type"`
	DistrictID   *uint      `form:"district_id"`
	EstateID     *uint      `form:"estate_id"`
	PropertyType *string    `form:"property_type"`
	StartPeriod  string     `form:"start_period" binding:"required,len=7"` // YYYY-MM
	EndPeriod    string     `form:"end_period" binding:"required,len=7"`   // YYYY-MM
	Limit        int        `form:"limit,default=12" binding:"min=1,max=60"`
}
